...
void InitializeBoard(GolCell *input, int gridWidth, int gridHeight, char *startingFile) {
	...
	for(int i = 0; i < gridWidth; i = i + 1) {
		for(int j = 0; j < gridHeight; j = j + 1) {
			...
			input[gridWidth * i + j] = (cell == '1') ? 1 : 0;
		}
	}
	fclose(file);
}
...
int main (int argc, char *argv[]) {
	...
	int BLOCKS_MAX = 256;
	int BLOCKS_X = min(BLOCKS_MAX, gridWidth / THREADS_X);
	int BLOCKS_Y = min(BLOCKS_MAX, gridHeight / THREADS_Y);
	int BLOCKS_Z = 1;
	...
	for(int i = 0; i < gridWidth; i = i + 1) {
		for(int j = 0; j < gridHeight; j = j + 1) {
			std::cout << output[j * gridWidth + i] ? '1' : '0';
		}
		std::cout << std::endl;
	}
}
